...
Note: Un buen inventario es la columna vertebral de la seguridad. "No puedes proteger lo que no conoces" es una máxima fundamental. El inventario debe incluir propietarios claros: cada activo tiene un responsable de negocio que decide su nivel de protección requerido. Utilizad herramientas de descubrimiento automatizado para hardware y software, pero no olvidéis los activos intangibles como conocimiento crítico en personas clave. Clasificad los datos por sensibilidad (público, interno, confidencial, secreto) para priorizar controles.
...
Note: El riesgo aceptable no es una cifra arbitraria: debe calcularse considerando el impacto en negocio. Usad el ejercicio BIA (Business Impact Analysis): ¿cuánto cuesta cada hora de inactividad de cada sistema crítico? Incluíd costes directos (ventas perdidas, productividad) e indirectos (daño reputacional, clientes que se van). Una caída de 4 horas de un e-commerce en temporada alta puede costar 100k€; la misma caída en una intranet administrativa tal vez 2k€. Estos números justifican inversiones diferenciadas. Documentad y haced firmar a dirección el apetito de riesgo: si luego no aprueban presupuesto, tenéis argumentos.
...
Note: Las iniciativas organizativas son la base: definen el marco de gobernanza. Sin políticas claras, cada persona interpreta la seguridad a su manera. La política de seguridad debe estar aprobada por dirección y comunicada a toda la organización. La SoA documenta qué controles de ISO 27001 aplican y cuáles no (con justificación). El proceso de gestión de incidentes asegura respuesta coordinada: quién, qué, cuándo. La concienciación es crítica: el 95% de brechas involucran error humano. Los contratos con proveedores deben incluir obligaciones de seguridad: muchas brechas ocurren a través de terceros.
...
Note: Las medidas técnicas son las que típicamente se asocian con "ciberseguridad", pero son ineficaces sin la base organizativa. MFA (multifactor authentication) es un quick win que previene el 99.9% de ataques de credenciales comprometidas según Microsoft. La segmentación de red limita el movimiento lateral de atacantes: si comprometen un equipo, no pueden moverse libremente. El SIEM centraliza logs permitiendo detectar patrones anómalos. Las copias sin pruebas de restauración son inútiles: muchas organizaciones han descubierto al sufrir ransomware que sus backups eran irrecuperables. El cifrado protege ante pérdida/robo de dispositivos.
...
Note: Los quick wins son fundamentales para generar momentum y credibilidad. Cuando la organización ve resultados rápidos, aumenta la confianza en el PDS. Estos proyectos también sirven para "calentar motores": el equipo practica la ejecución de proyectos, se rodean los procesos de cambio, se identifica resistencias. Un quick win exitoso facilita la aprobación de proyectos mayores. Comunicad ampliamente cada quick win completado: "Esta semana habilitamos MFA, reduciendo el riesgo de compromiso de credenciales en un 99%".
...
Note: Anticipad resistencias antes de la presentación. La resistencia suele venir de falta de conciencia sobre amenazas y percepción de que "IT es un coste". Vuestra misión es reencuadrar la seguridad como "protección del negocio". Usad analogías: "nadie cuestiona pagar un seguro contra incendios aunque nunca haya habido un incendio; la ciberseguridad es lo mismo". Si la objeción es económica real, ofreced un plan por fases: comenzad solo con quick wins y proyectos críticos, dejad los estratégicos para fase 2.
...
Note: Los KPIs deben ser SMART: específicos, medibles, alcanzables, relevantes, temporales. Evitad KPIs vanidad como "número de logs analizados" que no reflejan mejora en seguridad. Elegid 5-7 KPIs clave que se puedan medir mensualmente sin esfuerzo heroico. Automatizad la recopilación donde sea posible. Estableced metas realistas: no esperéis 100% de implantación en 3 meses. Visualizad los KPIs en dashboards accesibles al comité. Si un KPI no mejora, investigad: ¿falta de recursos? ¿resistencia? ¿objetivo irreal?
...
Note: La SoA (Statement of Applicability) es el corazón del SGSI ISO 27001. Es un Excel/tabla que lista los 93 controles de ISO 27002 (versión 2022; versión 2013 tenía 114 en Anexo A) y documenta su estado en vuestra organización. Ejemplo: Control "A.5.1 Políticas de seguridad": Estado = Implantado, Madurez = 3 (definido, documentado), Evidencia = "Política corporativa v2.3 aprobada 15/03/2024". Control "A.8.23 Filtrado web": Estado = No aplicable, Justificación = "No hay navegación web en servidores críticos, solo en estaciones de trabajo con protección endpoint". La SoA es auditable: el auditor pide las evidencias listadas.
...
Note: CMM (Capability Maturity Model) permite objetivar la evolución. Marcad nivel actual y nivel objetivo realista para 12-24 meses. Evitad saltos irreales: pasar de nivel 0 a 5 en un año es casi imposible. Más realista: de 1 a 3 en año 1, de 3 a 4 en año 2. La mayoría de organizaciones maduras están en nivel 3-4; nivel 5 es excelencia world-class. Usad la matriz para visualizar madurez por dominios: "Gestión de activos nivel 3, Gestión de incidentes nivel 2, Criptografía nivel 1" → priorizar incidentes y cripto. La matriz también sirve para comparar con benchmarks sectoriales.
...
Note: La elección del tratamiento depende del coste-beneficio. Mitigar un riesgo de 100k€ con controles de 20k€ tiene sentido; con controles de 200k€, no. Transferir mediante seguros es común para riesgos catastróficos de baja probabilidad (ej: seguro de ciberriesgo que cubre hasta 5M€). Evitar es raro pero a veces necesario: si ofrecer un servicio legacy inseguro genera más riesgo que beneficio, mejor cerrarlo. Aceptar es legítimo si el riesgo está bajo el umbral o el coste de mitigación es prohibitivo: pero debe documentarse y firmarse. Sin firma de dirección, el responsable IT carga con la culpa si el riesgo se materializa.
...
Note: Estos errores son extraordinariamente comunes. El enfoque solo tecnológico ("compramos un firewall carísimo y ya está") ignora que el 85% de brechas implican error humano. La desalineación con negocio: "vamos a parar el servidor de ventas 8 horas para parchearlo" sin considerar el impacto. El presupuesto insuficiente: aprobar un PDS de 100k€ pero solo dar 30k€. Sin seguimiento, los proyectos se eternizan. Imponer MFA sin explicar por qué genera rechazo. El perfeccionismo: "necesitamos 6 meses más de análisis antes de empezar" → la parálisis por análisis. Sin quick wins, el equipo y dirección pierden confianza.
